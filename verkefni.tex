...
\end{definition}
When studying a family of function it is helpful to know when the limit of a sequence in the family is also in the family.
\begin{lemma}
Let $K$ be a compact subset of $\mathbb{C}$ and $(f_n)_{n \in \mathbb{N}}$ be a sequence of continuous functions on $K$ with limit $f$.
Then $f$ is the uniform limit of $(f_n)_{n \in \mathbb{N}}$.
\end{lemma}
\begin{lemma}
Let $U$ be a subset of $\mathbb{C}$ and $(f_n)_{n \in \mathbb{N}}$ be a sequence of continuous functions on $K$ with uniform limit $f$.
Then $f$ is the continuous.
\end{lemma}
...
It can be shown \citep{greenkrantz} that if $h$ is holomorphic and $h'$ vanishes to order $k$ at $z_0$ then $h$ is $(k + 1)$-to-one in a neighbourhood of $z_0$. % TODO its on p. 165 in G&K
...
\end{theorem}
\begin{proof}
A proof of this is rather involved and can be found in \citep{greenkrantz}. % TODO add where in citation
\end{proof}
\begin{corollary}
...
\]
and say it is \emph{bounded} if said norm is finite.
\end{definition}
\begin{theorem}
...
\end{enumerate}
\end{theorem}
We will use the terms `bounded' and `continuous' interchangeably when talking about linear mappings between normed vector spaces, due to this theorem.
A proof of this theorem can be found in \citep{rudin2}.
\subsection{Hahn-Banach}
% TODO sjpall
There are many related theorems going by the name `Hahn-Banach Theorem'.
These are sometime split in two groups, `separation theorems' and `extension theorems'.
...
\begin{theorem}[The Riesz reprentation theorem for bounded linear functionals on $C_0(X)$ with locally compact $X$]
...
Both of these things can be shown by demonstrating that the series converges uniformly.
...
To prove the result for a general $T$ we first let $\Phi: T \rightarrow S$ be the map provided to us by \label{contrmp}.
...
The first and last equalities hold because the euclidean metric is rotationally invariant, and the second equality holds because the distance from $z$ to a real number $a$ is the same distance from $\overline{z}$ to $a$.
We now obtain
\begin{align*}
...
Let $X$ be a compact Hausdorff space,
	...
	$f$ be a continues function on $S$,
...
on $S$.
\end{proof}
\begin{proof}[Alternate proof of \ref{rudincarleson}]
Let
	$X = \mathbb{T}$,
