...
Section \ref{section1} is dedicated to prove the F. and M. Riesz theorem, which states a sufficient condition for a measure to be absolutely continuous with regards to the Lebesgue-measure on the unit circle.
...
In section \ref{section5} we look at a way to relax the condition on the set in Bishop's theorem.
The set in Bishop's theorem is conditioned such that all continuous functions thereon can be extended, that is the is independent of the function we want to extend.
...
Hún gefur okkur nægjanlegt skilyrði á hlutmengi í einingarhringnum í $\mathbb{C}$ þannig að framlengja megi sérhvert samfallt fall á menginu í samfellt fall á lokuðu einingarhringskífunni sem er fágað á opnu einingarhringskífunni.
...
Við skoðum síðan í kafla \ref{section4} hvernig setning Bishops getur hjálpað okkur að flokka lokuð hlutmengi jaðars einingahrings $\mathbb{C}^n$.

Í kafla \ref{section5} skoðum hvernig slaka má á skilyrðinnum í setningu Bishops.
Mengið í setningu Bishops er skilyrt þanni að öll samfelld föll á því megi framlengja, það er að segja skilyrðin eru óháð fallinu sem við viljum framlengja.
...
\[
	\int_S F\ d\mu = 0
\]
holds for all $F$ in $B$ and $\mu$ in $B^{\bot}$ then there exists a function $F \in B$ such that $F = f$ on $S$ and $|F| < \Xi$ on $X$.
\end{theorem}
...
\[
	\int_S F\ d\mu = 0
\]
holds for all $F$ in $\mathcal{A}^n$ and $\mu$ in $\mathcal{A}^n|_{\mathbb{T}}^{\bot}$ then there exists a function $F \in \mathcal{A}^n$ such that $F = f$ on $S$.
\end{theorem}
...
\indexitem{Holomorhpic in $\mathbb{C}^n$}{index9}
