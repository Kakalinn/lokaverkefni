...
It states a sufficient condition on a subset of the unit circle in $\mathbb{C}$ such that every continuous function thereon can be extended to a continuous function on the closed unit disk whose restriction to the open unit disk is holomorphic.
...
We also look at the role of Bishop's theorem in the classification of closed subsets of the boundary of the unit ball in $\mathbb{C}^n$.
...
Einnig er athugað hvernig setning Bishops getur hjálpað okkur að flokka lokuð hlutmengi jaðars einingarkúlu $\mathbb{C}^n$.
Loks skoðum hvernig slaka má á skilyrðum setningar Bishops.
\vfill
\newpage
...
We start by proving the F. and M. Riesz theorem in Section \ref{section1}, which states a sufficient condition on a measure on the unit circle such that it is absolutely continuous with regards to the Lebesgue-measure on the unit circle.
...
We shall always assume that measure are complex Borel measures, that is, $\mathcal{F}$ is the Borel $\sigma$-algebra abd the measure take complex values, unless stated otherwise.
...
\begin{definition}
\label{index7}
A closed curve $\gamma \colon [a, b] \rightarrow \mathbb{C}$ is said to be a \emph{Jordan curve} or a \emph{simple curve} if it is injective on the half-open interval $[a, b[$.
\end{definition}
...
Let $U$ and $V$ be domains in $\mathbb{C}$ each bounded by a Jordan curve and $f: U \rightarrow V$ be a conformal mapping.
Then there exists a continuous injection $\hat{f}: \overline{U} \rightarrow \overline{V}$ that extends $f$.
\end{theorem}
\label{index8}
...
The following discussion is dedicated to the existence of convergent subsequences of a bounded sequence of bounded linear functionals.
We will therefore, for some infinite $S \subset \mathbb{N}$, define the sequence $(a_n)_{n \in S}$ to be $(a_{s_n})_{n \in \mathbb{N}}$, where $s_k$ is the $k$-th smallest element of $S$.
...
For $f \in L^1(\mathbb{T})$ we define \emph{the Poisson integral of $f$} as a function on $\mathbb{D}$ by
\[
	P[f](z) = \frac{1}{2\pi} \int_{-\pi}^{\pi} P(z, e^{it}) f(e^{it})\ dt.
\]

For a measure $\mu$ on $\mathbb{T}$ we define \emph{the Poisson integral of $\mu$} as a function on $\mathbb{D}$ by
\[
...
This notation is also more readable than $\int_{[-\pi, \pi]}$ and writing $\int_{-\pi}^{\pi}$ does not take into consideration the possibility of singletons having a non-zero measure with regards to $\mu$.
...
That is, if $P[d\mu] = P[d\lambda]$ then $\mu = \lambda$.
\end{proof}
...
To prove the result for a general $T$ we first let $\Phi: T \rightarrow S$ be the map provided to us by Corollary \ref{contrmt}.
...
\indexitem{Hölder's inequality}{index16}
%
