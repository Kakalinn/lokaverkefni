...
% COMMENT(gardar): Mér finnst vanta aðeins meiri yfirsýn yfir hvað þú ert að fara tala um
%  í ritgerðinni hér og mín vegna mætti færa þessar skilgreiningar í Preliminaries t.d. með
%  því að bæta við  kafla '2.0 notation and conventions'.
% TODO define 'extends'
...
\begin{definition}
% COMMENT(gardar): Er R yfirstrik alveg vel þekkt?
...
% COMMENT(gardar): s/great/significant
...
% COMMENT(gardar): The usual definition of null sets would label $X$ as a $\mu$-null set.
...
% COMMENT(gardar): s/component/components
...
% COMMENT(gardar): s/will be called/is called
...
% COMMENT(gardar): optional: conformal hefur ekki verið skilgreint þegar hér kemur við sögu, mætti kannski nefna hvað það þýðir. Ahh sé að þetta er skilgreint neðar, þannig þetta er kannski alveg OK.
...
% COMMENT(gardar): maybe?: Before proving the Riemann Mapping Theorem we need to define conformality.
...
% COMMENT(gardar): s/If U ..., .../If U ... and ....
...
% COMMENT(gardar): Since $\Phi_2$ is conformal $\Phi_2^{-1}$ is also bijective, holomorphic with an holomorphic inverse, meaning $\Phi_2^{-1}$ is also conformal.
% COMMENT(gardar): s/then/therefore
...
% COMMENT(gardar): s/achieve the desired theorem/prove the desired result
...
% COMMENT(gardar): Gætir fært skilgrininguna á B_r(z) í 'notations' undirkafla í byrjun.
...
% COMMENT(gardar): Þarf f^{-1} ekki líka að vera samfelld til að sömu rök virki?
...
% COMMENT(gardar): Mér finnst það mætti bæta við setningu hérna sem útskýrir aðeins pointið með þessari efnisgrein.
Second, let
	$U$ and $V$ be open sets in $\mathbb{C}$,
	$\gamma: [0, 1] \rightarrow \partial \mathbb{D}$ a Jordan curve,
	$f: U \rightarrow V$ be homeomorphism,
% COMMENT(gardar): Þarf ekki að grf að \partial D \subset U til að \lambda sé velskilgreint?
...
% COMMENT(gardar): nit: Etv setja þessa skilgreiningu í \begin{definition} etc.
...
% COMMENT(gardar): continuos function $f: X \to X$?
...
% COMMENT(gardar): Vantar þá ekki skilyrði í skilgreininguna um að föllin í C_0(X) séu samfelld?
...
Let
% COMMENT(gardar): etv á locally convex að vera locally compact?
	$X$ be a locally convex normed vector space,
...
% COMMENT(gardar): ... measure $\mu$, choose ... -> measure $\mu$. Choose
...
% COMMENT(gardar): Það kemur skilgreining beint á eftir þessari setningu, kannski færa hana neðar?
...
% COMMENT(gardar): Persónulega mundi ég skrifa næstu línu sem: \int_{-\pi}^{\pi} r^0 e^{0}
	&= \int_{-\pi}^{\pi}\ dt\\
	&= 2\pi.
\end{align*}
...
We have that
\begin{align*}
% COMMENT(gardar): Hér er u en ekki u_r eins og í setningunni. Á það að vera svoleiðis?
	\|u\|_1
...
\[
% COMMENT(gardar): Á tölugildið ekki að vera inn í max-inu?
