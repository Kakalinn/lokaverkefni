...
The measure $\lambda$ mentioned earlier will then fulfill
\[
...
% TODO 'the theorem will be proved as Rudin did it in his ortiginal paper \citep{...}'
...
\begin{lemma}
\label{mylemma}
Let $H$ be a closed set of Lebesgue-measure $0$.
...
We will define functions $f: [0, 1] \setminus H \rightarrow [0, \infty[$ and $F: [0, 1] \setminus H \rightarrow \mathbb{R}$ by
\begin{align*}
	f_H(x) &= \frac{H_H(x) - V_H(x)}{\sqrt{(x - V_H(x))(H_H(x) - x)}}, \text{ and}, \\
	F_H(x) &= -\tan^{-1}\left ( \frac{1}{2} \sqrt{\frac{V_H(x) - x}{x - H_H(x)}} - \frac{1}{2}\sqrt{\frac{x - H_H(x)}{V_H(x) - x}} \right ).
\end{align*}
One rather useful property of this pair of functions is that $F_H' = f_H$.

To see where these functions come from and what they look like let's first consider the case of what $f$ looks like when $H = \{0, 1\}$.
Then 
\[
	f_H(x) = \frac{1}{\sqrt{x(1 - x)}}.
\]
this is a scaled version of the probability density function of an arcsine distribution... % TODO add maybe a citation to the BM book and a nice little picture.


% TODO is f_H measurable? L^1?
...
Let $u_H(z)$ be the Poisson integral of the function from the above lemma. % TODO add ref fix up this line
...
\begin{lemma}
\label{rudinlemma3}
If $f$ is a continue function on $E$ which maps $E$ into
\[
	S = \{z;\ |\max(\Re z, \Im z)| \leq 1\}, % TODO make this definition global (to the section)?
\]
then there exists a sequence $(f_n)_{n \in \mathbb{N}}$ of simple continuous function on $E$ such that
\[
...
We again use \ref{FMRlemma31} and the Riesz representation theorem to show that there exists a function $f$ in $L^2(\mathbb{T})$ with $\|f\|_2 \leq M$ and
\[
...
Iterating this process yields a sequence $(F_n)_{n \in \mathbb{N}}$ from $B$ that fulfill $\|F_n\| < 2^{1 - n}\lambda$ for $n > 1$ and
\[
