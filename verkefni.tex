...
\subsection{Caratheodory} % TODO Spell checka nafnið
...
The desired conformal mapping from $U$ to $V$ is then $\Phi_1^{-1} \circ \Phi_2$.
\end{proof}
\begin{corollary}
If $K$ is homeomorphic to $\overline{\mathbb{D}}$ then there exists a homeomorphism $\Phi:\ \overline{\mathbb{D}} \rightarrow K$ such that its restriction to $\mathbb{D}$ is a conformal mapping.
\end{corollary}
\begin{proof}
% Nota Caratheodory. Samskeyting stikunar á jaðri D og grannmótunar á milli D og K ætti að vera Jordan stikun á jaðri K, vonandi.
TODO
\end{proof}
